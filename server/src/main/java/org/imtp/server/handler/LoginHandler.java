...
//        LoginInfo loginInfo = loginRequest.getLoginInfo();
//        User user = chatService.findByUsername(loginInfo.getUsername());
...
//            ChannelContext channelContext = ChannelContextHolder.channelContext();
//            channelContext.addChannel(channel.id().asLongText(),new IMChannelSession(channelHandlerContext.channel(),user.getId().toString()));
//            //建立channel与用户之间的关系
//            AttributeKey<String> attributeKey = AttributeKey.valueOf(ProjectConstant.CHANNEL_ATTR_LOGIN_USER);
//            channelHandlerContext.channel().attr(attributeKey).set(user.getId().toString());
...
//        }else {
//            channelHandlerContext.channel().writeAndFlush(new LoginResponse(LoginState.FAIL,Long.valueOf(loginInfo.getUsername())));
//            log.info("用户:{} 用户名或密码错误",loginInfo.getUsername());
//        }
//    }
//}
