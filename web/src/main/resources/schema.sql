...
(
    ...
    `total_size`           bigint                                                        DEFAULT NULL COMMENT '文件总大小 单位:字节',
    `total_chunk`          int                                                           DEFAULT NULL COMMENT '总块数',
    `chunk_size`           int                                                           DEFAULT NULL COMMENT '每块大小 单位:字节',
    `uploaded_chunk_count` int                                                           DEFAULT NULL COMMENT '已上传的分片数',
    `etag`                 varchar(255) COLLATE utf8mb4_general_ci                       DEFAULT NULL COMMENT '文件标识',
    `access_url`           varchar(255) COLLATE utf8mb4_general_ci                       DEFAULT NULL COMMENT '访问url',
    `status`               varchar(32) COLLATE utf8mb4_general_ci                        DEFAULT NULL COMMENT 'PENDING, COMPLETED, FAILED',
    `storage_type`         varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci  DEFAULT NULL COMMENT '存储类型 LOCAL 本地存储、 MINIO 对象存储',
    `create_time`          datetime                                                      DEFAULT NULL,
    `update_time`          datetime                                                      DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `upload_id_idx` (`upload_id`) USING BTREE
) ENGINE = InnoDB
